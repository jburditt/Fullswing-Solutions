...
<div id="respond">
	...
	<?php if ( get_option( 'comment_registration' ) && ! $user_ID ) : //If registration required & not logged in. ?>
		...
		<p><?php _e( 'You must be', 'woothemes' ); ?> <a href="<?php echo esc_url( wp_login_url( get_permalink() ) ); ?>" rel="nofollow"><?php _e( 'logged in', 'woothemes' ); ?></a> <?php _e( 'to post a comment.', 'woothemes' ); ?></p>
	...
	<?php else : //No registration required ?>
		...
		<?php if ( $user_ID ) : //If user is logged in ?>
			...
			<p><?php _e( 'Logged in as', 'woothemes' ); ?> <a href="<?php echo esc_url( admin_url( '/profile.php' ) ); ?>"><?php echo $user_identity; ?></a>. <a href="<?php echo esc_url( wp_logout_url() ); ?>" title="<?php esc_attr_e( 'Log out of this account', 'woothemes' ); ?>"><?php _e( 'Logout', 'woothemes' ); ?> &raquo;</a></p>
		...
		<?php else : //If user is not logged in ?>
			...
			<p>
				...
				<label for="email"><?php _e( 'Mail (will not be published)', 'woothemes' ); ?>  <?php if ( $req ) echo '(' . __( 'required', 'woothemes' ) . ')'; ?></label>
			</p>
